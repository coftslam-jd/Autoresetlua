...
local function TP(name)
	...
	if t and t.Character then
		thrp = t.Character:FindFirstChild("HumanoidRootPart");
	end
...
main.BackgroundColor3 = Color3.fromRGB(24, 20, 26);
...
top.BackgroundColor3 = Color3.fromRGB(18, 15, 19);
top.BackgroundTransparency = TR;
...
closeBtn.BackgroundColor3 = Color3.fromRGB(40, 35, 42);
closeBtn.BackgroundTransparency = TR;
closeBtn.Text = "X";
...
sidebar.BackgroundColor3 = Color3.fromRGB(20, 17, 21);
sidebar.BackgroundTransparency = TR;

local sideList = Instance.new("Frame", sidebar);
sideList.Size = UDim2.new(1, -10, 1, -10);
sideList.Position = UDim2.new(0, 5, 0, 5);
sideList.BackgroundTransparency = 1;

local sl = Instance.new("UIListLayout", sideList);
...
local function sideBtn(t, o)
	...
	b.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
...
content.Size = UDim2.new(1, -74, 1, -46);
content.Position = UDim2.new(0, 69, 0, 40);
...
local function makeInput(p, ph, o, d)
	...
	t.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
	t.BackgroundTransparency = TR;
...
statusLabel.Text = U(1043,1086,1090,1086,1074);
...
local p1 = makePanel(2);
p1.Visible = true;

local selLabel = Instance.new("TextLabel", p1);
selLabel.BackgroundTransparency = 1;
selLabel.Size = UDim2.new(1, 0, 0, 20);
selLabel.Text = U(1042,1099,1073,1088,1072,1085,58,32,45);
selLabel.TextColor3 = Color3.fromRGB(190, 190, 190);
...
scroll.BackgroundColor3 = Color3.fromRGB(18, 15, 19);
scroll.BackgroundTransparency = TR;
scroll.ScrollBarThickness = 4;
scroll.CanvasSize = UDim2.new(0, 0, 0, 0);
scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y;
scroll.LayoutOrder = 2;

local scrollLL = Instance.new("UIListLayout", scroll);
scrollLL.Padding = UDim.new(0, 4);
	...
	for _, pl in ipairs(players) do
		if pl ~= LP then
			...
			b.MouseButton1Click:Connect(function()
				targetName = pl.Name;
				local nameTxt = U(1042,1099,1073,1088,1072,1085,58,32);
				selLabel.Text = nameTxt .. pl.Name;
			end);
		end
	end
end
...
statsWin.Size = UDim2.new(0, 220, 0, 150);
statsWin.Position = UDim2.new(0, 600, 0, 120);
statsWin.BackgroundColor3 = Color3.fromRGB(24, 20, 26);
statsWin.BackgroundTransparency = TR;
statsWin.Visible = false;
statsWin.Active = true;
statsWin.Draggable = true;
local statsWinCorner = Instance.new("UICorner", statsWin);
statsWinCorner.CornerRadius = UDim.new(0, 10);
...
swTitle.Size = UDim2.new(1, 0, 0, 26);
swTitle.Text = U(1057,1090,1072,1090,1080,1089,1090,1080,1082,1072);
...
swTime.Position = UDim2.new(0, 8, 0, 30);
swTime.Size = UDim2.new(1, -16, 0, 20);
swTime.TextColor3 = Color3.fromRGB(220, 220, 220);
swTime.Font = Enum.Font.Gotham;
swTime.TextXAlignment = Enum.TextXAlignment.Left;
fit(swTime);

local swMoney = swTime:Clone();
swMoney.Parent = statsWin;
swMoney.Position = UDim2.new(0, 8, 0, 58);
...
swToggleBtn.MouseButton1Click:Connect(function()
	statsWin.Visible = not statsWin.Visible;
end);

local swSizeRow = Instance.new("Frame", p2);
swSizeRow.Size = UDim2.new(1, 0, 0, 30);
swSizeRow.BackgroundTransparency = 1;
swSizeRow.LayoutOrder = 2;

local swMinus = Instance.new("TextButton", swSizeRow);
swMinus.Size = UDim2.new(0, 54, 1, 0);
swMinus.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
swMinus.BackgroundTransparency = TR;
swMinus.Text = "-";
swMinus.TextColor3 = Color3.new(1, 1, 1);
swMinus.Font = Enum.Font.GothamBold;
fit(swMinus);
local swMinusCorner = Instance.new("UICorner", swMinus);
swMinusCorner.CornerRadius = UDim.new(0, 6);

local swPlus = Instance.new("TextButton", swSizeRow);
swPlus.Size = UDim2.new(0, 54, 1, 0);
swPlus.Position = UDim2.new(0, 62, 0, 0);
swPlus.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
swPlus.BackgroundTransparency = TR;
swPlus.Text = "+";
swPlus.TextColor3 = Color3.new(1, 1, 1);
swPlus.Font = Enum.Font.GothamBold;
fit(swPlus);
local swPlusCorner = Instance.new("UICorner", swPlus);
swPlusCorner.CornerRadius = UDim.new(0, 6);

swMinus.MouseButton1Click:Connect(function()
	statsScale.Scale = math.clamp(statsScale.Scale - 0.1, 0.5, 1.8);
end);

swPlus.MouseButton1Click:Connect(function()
	statsScale.Scale = math.clamp(statsScale.Scale + 0.1, 0.5, 1.8);
end);
...
cTime.Size = UDim2.new(1, 0, 0, 18);
cTime.Text = "00:00:00";
cTime.TextColor3 = Color3.fromRGB(200, 200, 200);
cTime.Font = Enum.Font.Gotham;
cTime.TextXAlignment = Enum.TextXAlignment.Left;
cTime.LayoutOrder = 4;
fit(cTime);

local cMoney = cTime:Clone();
cMoney.Parent = p2;
cMoney.LayoutOrder = 5;
...
sizeLabel.Text = U(1056,1072,1079,1084,1077,1088,58,32,49,48,48,37);
sizeLabel.TextColor3 = Color3.fromRGB(210, 210, 210);
sizeLabel.Font = Enum.Font.Gotham;
sizeLabel.TextXAlignment = Enum.TextXAlignment.Left;
sizeLabel.LayoutOrder = 1;
fit(sizeLabel);

local sizeRow = Instance.new("Frame", p3);
...
mMinus.Size = UDim2.new(0, 54, 1, 0);
mMinus.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
mMinus.BackgroundTransparency = TR;
mMinus.Text = "-";
mMinus.TextColor3 = Color3.new(1, 1, 1);
mMinus.Font = Enum.Font.GothamBold;
fit(mMinus);
local mMinusCorner = Instance.new("UICorner", mMinus);
mMinusCorner.CornerRadius = UDim.new(0, 6);

local mPlus = Instance.new("TextButton", sizeRow);
mPlus.Size = UDim2.new(0, 54, 1, 0);
mPlus.Position = UDim2.new(0, 62, 0, 0);
mPlus.BackgroundColor3 = Color3.fromRGB(40, 34, 44);
mPlus.BackgroundTransparency = TR;
mPlus.Text = "+";
mPlus.TextColor3 = Color3.new(1, 1, 1);
mPlus.Font = Enum.Font.GothamBold;
fit(mPlus);
local mPlusCorner = Instance.new("UICorner", mPlus);
mPlusCorner.CornerRadius = UDim.new(0, 6);

local function setScale(v)
	scale.Scale = math.clamp(v, 0.6, 1.6);
	local pct = tostring(math.floor(scale.Scale * 100));
	sizeLabel.Text = U(1056,1072,1079,1084,1077,1088,58,32) .. pct .. "%";
end

mMinus.MouseButton1Click:Connect(function()
	setScale(scale.Scale - 0.1);
end);

mPlus.MouseButton1Click:Connect(function()
	setScale(scale.Scale + 0.1);
end);

local afkRow = Instance.new("Frame", p3);
afkRow.Size = UDim2.new(1, 0, 0, 28);
afkRow.BackgroundTransparency = 1;
afkRow.LayoutOrder = 3;

local afkLabel = Instance.new("TextLabel", afkRow);
afkLabel.BackgroundTransparency = 1;
afkLabel.Size = UDim2.new(1, -54, 1, 0);
afkLabel.Text = U(1040,1085,1090,1080,45,1040,1060,1050);
afkLabel.TextColor3 = Color3.fromRGB(210, 210, 210);
...
afkBtn.Size = UDim2.new(0, 48, 0, 24);
afkBtn.Position = UDim2.new(1, -48, 0.5, -12);
afkBtn.BackgroundTransparency = TR;
afkBtn.Font = Enum.Font.GothamBold;
afkBtn.TextColor3 = Color3.new(1, 1, 1);
fit(afkBtn);
local afkBtnCorner = Instance.new("UICorner", afkBtn);
afkBtnCorner.CornerRadius = UDim.new(0, 10);

local function refreshAfk()
	if antiAfk then
		afkBtn.Text = U(1042,1050,1051);
		afkBtn.BackgroundColor3 = Color3.fromRGB(45, 140, 80);
	else
		afkBtn.Text = U(1042,1067,1050,1051);
		afkBtn.BackgroundColor3 = Color3.fromRGB(90, 45, 45);
	end
end
...
local function stop(msg)
	running = false;
	paused = false;
	startBtn.Text = U(1057,1058,1040,1056,1058);
	startBtn.BackgroundColor3 = Color3.fromRGB(45, 140, 80);
	if msg then
...
local function chain(char)
	...
	local ok = TP(targetName);
	if not ok then
		stop(U(1053,1077,32,1091,1076,1072,1083,1086,1089,1100,32,1090,1077,1083,1077,1087,1086,1088,1090,1080,1088,1086,1074,1072,1090,1100,1089,1103));
		return;
	end
	task.wait(0.3);
	if not running then
...
LP.CharacterAdded:Connect(function(char)
	deaths = deaths + 1;
	if not running or resetCount == 0 then
		return;
	end
	if target > 0 and resetCount >= target then
		stop(U(1043,1086,1090,1086,1074,1086,58,32,1074,1089,1077));
		return;
	end
	task.spawn(function()
		chain(char);
...
Players.PlayerRemoving:Connect(function(p)
	if p.Name == targetName and running then
		...
		startBtn.Text = U(1055,1040,1059,1047,1040);
		startBtn.BackgroundColor3 = Color3.fromRGB(150, 120, 40);
	end
	refreshList();
end);
...
Players.PlayerAdded:Connect(function(p)
	...
	if p.Name == targetName and paused then
		...
		startBtn.Text = U(1057,1058,1054,1055);
...
startBtn.MouseButton1Click:Connect(function()
	...
	if targetName == "" then
		statusLabel.Text = U(1042,1099,1073,1077,1088,1080,32,1080,1075,1088,1086,1082,1072);
		return;
	end
	target = tonumber(resetsInput.Text) or 0;
	local ok = TP(targetName);
	if not ok then
		statusLabel.Text = U(1053,1077,32,1091,1076,1072,1083,1086,1089,1100,32,1090,1077,1083,1077,1087,1086,1088,1090,1080,1088,1086,1074,1072,1090,1100,1089,1103);
		return;
	end
	task.wait(0.5);
	resetCount = 1;
...
task.spawn(function()
	while true do
		...
		if running then
			...
			t = string.format("%02d:%02d:%02d", e // 3600, (e % 3600) // 60, e % 60);
		end
